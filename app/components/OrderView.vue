...
import { commands, type OrderWithClient } from "@/bindings";
...
onMounted(async () => {
  ...
  if (getResult.status === "ok" && getResult.data.data) {
    order.value = getResult.data.data as unknown as OrderWithClient;
  } else {
    Logger.error(`ERROR GET ORDER: ${JSON.stringify(getResult.error)}`);
  }
...
const clientDetails = computed(() => {
  ...
  return [
    order.value.email || t("placeholders.no-email"),
    order.value.address || t("placeholders.no-address"),
    order.value.phone_number || t("placeholders.no-phone"),
  ];
});
  ...
  <div class="h-full w-full max-w-[860px]">
    ...
    >
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        <div class="space-y-1">
          ...
          <h2 class="text-xl font-semibold">
            {{ t("titles.orders.view") }} {{ props.identifier }}
          </h2>
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto">
        ...
        <div v-else-if="order" class="w-full h-full px-5 py-6 sm:px-6">
            ...
            <div class="space-y-6 border-b border-slate-200 py-6">
              <div class="grid gap-4 text-sm sm:grid-cols-2">
                ...
                <div class="space-y-2 border-b border-slate-100 pb-3 sm:border-b-0 sm:pb-0">
                  ...
                  <div class="flex items-center h-9 font-medium text-slate-900">
                    {{ order.created_at ? d(new Date(order.created_at), "long") : "--" }}
                  </div>
                </div>
              </div>
      ...
      <div class="border-t border-slate-200 px-5 py-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="text-sm text-slate-500">
            {{ order?.full_name || t("placeholders.no-client-selected") }}
          </div>
