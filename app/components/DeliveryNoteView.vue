...
const { t, d, n } = useI18n();
const deliveryNote = ref<any>(null);
const isLoading = ref(true);

onMounted(async () => {
  const getResult = await commands.getDeliveryNote(props.id);
  if (getResult.status === "error") {
    Logger.error(`ERROR GET DELIVERY NOTE: ${JSON.stringify(getResult.error)}`);
  } else if (getResult.data.data) {
    deliveryNote.value = getResult.data.data;
  }
  isLoading.value = false;
});

const subtotal = computed(() => {
  if (!deliveryNote.value?.items) return 0;
  return deliveryNote.value.items.reduce((sum: number, item: any) => {
  ...
  <div class="h-full w-full max-w-[860px]">
    ...
    >
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        <div class="space-y-1">
          ...
          <h2 class="text-xl font-semibold">
            {{ t("titles.delivery-notes.view") }} {{ props.identifier }}
          </h2>
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto">
        ...
        <div v-else-if="deliveryNote" class="w-full h-full px-5 py-6 pb-8 sm:px-6">
            ...
            <div class="space-y-6 border-b border-slate-200 py-6">
              <div class="space-y-2">
                ...
                <div class="flex items-center h-9 font-medium text-slate-900">
                  {{ deliveryNote.created_at ? d(new Date(deliveryNote.created_at), "long") : "--" }}
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-1 text-sm leading-6 text-slate-500">
                  ...
                  <p v-if="deliveryNote.client?.email">{{ deliveryNote.client.email }}</p>
                  <p v-if="deliveryNote.client?.address">{{ deliveryNote.client.address }}</p>
                  <p v-if="deliveryNote.client?.phone_number">{{ deliveryNote.client.phone_number }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="py-6">
              ...
              <div class="border-y border-slate-200">
                ...
                <div class="divide-y divide-slate-200">
                  <div
                    v-for="(item, idx) in deliveryNote.items"
                    :key="idx"
      ...
      <div class="border-t border-slate-200 px-5 py-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="text-sm text-slate-500">
            {{ deliveryNote?.client?.full_name || t("placeholders.no-client-selected") }}
          </div>
