...
async function deleteTheSupplier() {
  try {
    const result = await commands.deleteSupplier(props.id);
  ...
  <Card class="card-modal-shell max-w-xl">
    <div class="card-modal-header">
      <div class="card-modal-header-inner">
        <div class="space-y-1">
          ...
          <h2 class="card-modal-title">{{ t("titles.suppliers.delete") }}</h2>
          <p class="card-modal-description">{{ t("modalDescriptions.suppliers.delete") }}</p>
        </div>
    ...
    <div class="card-modal-footer">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        ...
        <Button @click="deleteTheSupplier">
          {{ t("buttons.confirme") }}
        </Button>
      </div>
    </div>
