...
onMounted(async () => {
  const getResult = await commands.getCreditNote(props.id);
  if (getResult.status === "error") {
    Logger.error(`ERROR GET CREDIT NOTE: ${JSON.stringify(getResult.error)}`);
  } else if (getResult.data.data) {
    creditNote.value = getResult.data.data;

    const invoiceResult = await commands.getInvoice(creditNote.value.invoice_id);
    if (invoiceResult.status === "ok" && invoiceResult.data.data) {
      invoice.value = invoiceResult.data.data as InvoiceWithClient;
    }
  }
...
const subtotal = computed(() => {
  if (!creditNote.value?.items) return 0;
  return creditNote.value.items.reduce((sum: number, item: any) => {
  ...
  <div class="h-full w-full max-w-[860px]">
    ...
    >
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        <div class="space-y-1">
          ...
          <h2 class="text-xl font-semibold">
            {{ t("titles.credit-notes.view") }} {{ props.identifier }}
          </h2>
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto">
        ...
        <div v-else-if="creditNote" class="w-full h-full px-5 py-6 pb-8 sm:px-6">
            ...
            <div class="space-y-6 border-b border-slate-200 py-6">
              <div class="grid gap-4 text-sm sm:grid-cols-2">
                <div class="space-y-2 border-b border-slate-100 pb-3 sm:border-b-0 sm:pb-0">
                  <span class="text-slate-500">{{ t("fields.reason") }}</span>
                ...
                <div class="space-y-2 border-b border-slate-100 pb-3 sm:border-b-0 sm:pb-0">
                  ...
                  <div class="flex items-center h-9 font-medium text-slate-900">
                    {{ creditNote.created_at ? d(new Date(creditNote.created_at), "long") : "--" }}
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-1 text-sm leading-6 text-slate-500">
                  ...
                  <p v-if="creditNote.client?.email">{{ creditNote.client.email }}</p>
                  <p v-if="creditNote.client?.address">{{ creditNote.client.address }}</p>
                  <p v-if="creditNote.client?.phone_number">{{ creditNote.client.phone_number }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="py-6">
              ...
              <div class="border-y border-slate-200">
                ...
                <div class="divide-y divide-slate-200">
                  <div
                    v-for="(item, idx) in creditNote.items"
                    :key="idx"
      ...
      <div class="border-t border-slate-200 px-5 py-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="text-sm text-slate-500">
            {{ creditNote?.client?.full_name || t("placeholders.no-client-selected") }}
          </div>
