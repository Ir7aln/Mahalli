...
import { DELIVERY_NOTE_STATUSES } from "~/consts";
...
const { data: deliveryNote } = await useAsyncData(async () => {
  const result = await commands.getDeliveryNote(id as string);
  if (result.status === "error") {
    showErrorToast(result.error);
    Logger.error(`ERROR DELIVERY NOTE DETAILS: ${JSON.stringify(result.error)}`);
    return null;
  }
...
async function handleGeneratePdf() {
  try {
    const pdfDataUri = await generatePdf(deliveryNote.value, "delivery-note");
    if (pdfDataUri) {
      pdfContent.value = pdfDataUri;
    }
  } catch (err: any) {
    showErrorToast(err);
    Logger.error(`ERROR DELIVERY NOTE PDF: ${err.error ? err.error : err.message}`);
  }
}
...
async function updateConfig(configAndValues: any) {
  ...
  deliveryNote.value = { ...deliveryNote.value, ...documentValues };
...
<template>
  <main class="w-full h-full flex gap-2 min-h-[calc(100vh-68px)]">
    ...
    <TemplateForm
      ...
      :statues="DELIVERY_NOTE_STATUSES"
